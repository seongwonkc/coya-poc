document.addEventListener('DOMContentLoaded', () => {
    ...
    async function takeTurn(userAction) {
        ...
        const promptForAI = `
            ...
            INSTRUCTIONS: 1. Narrate the outcome. 2. Determine consequences ('durationMonths', 'health', 'wealth'). 3. Suggest 3 relevant 'action_ids' from this list: [${Object.keys(gameActions.actions).join(', ')}].
        ...
        for (let i = 1; i <= 17; i++) {
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    throw new Error(errorData.error || `API Response not OK (${response.status})`);
                }
                const data = await response.json();
                ...
                storyContainer.innerHTML += `<p class="ai-text event-text">Error simulating age ${character.age}: ${error.message}. The year passed uneventfully.</p>`;
                storyContainer.scrollTop = storyContainer.scrollHeight;
            }
        }
    ...
    function generateInitialLifeEvents() {
        ...
        if (Math.random() < incarcerationChance) {
            ...
            character.background.isFromSingleParentHousehold = true;
            character.health -= 10;
        }
    }
    ...
    function getCharacterBiography() {
        ...
        if (character.background.isHighNetWorth) {
            bio += 'Comes from a high net worth family. ';
        }
        return bio;
    }
    ...
    function displayChoices(choices) {
        ...
        choices.forEach(actionId => {
            ...
            if (!action) {
                console.warn(`Could not find action for ID: ${actionId}`);
                return;
            };
    ...
    function resolveAction(actionId) {
        ...
        if (action.requirements) {
            if (action.requirements.min_wealth && character.wealth < action.requirements.min_wealth) {
                ...
                takeTurn(`(I was unable to afford '${action.label}')`);
                return;
            }
        }
